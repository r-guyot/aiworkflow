...
qdrant_upsert_points <- function(conn, points=list(), collection_name, generate_id=TRUE) {
  ...
  if (!identical(points, list())) {
    ...
    length_input_vector <- length(points[[1]][[1]]$vector)
    
    # check size of the collection
    coll <- qdrant_get_collection_details(conn = conn, collection_name = collection_name)
    coll_vec_size <- coll$result$config$params$vectors$size
    
    #aborting if does not match the vector size of the collection. reason: this is not detected in the qdrant api result.
    if (length_input_vector != coll_vec_size) {
      cli::cli_abort("The size of the vector to insert ({length_input_vector}) is not compatible with the vector size ({coll_vec_size}) of the {collection_name} collection. Aborting.")
    }
    ...
    if (res$status_code==200) {
      ...
      # not enough, need to check if collection has correct vector size
...
set_qdrant_collection <- function(workflow_obj, 
                                  collection_name=NA_character_,
                                  vector_size=NA_integer_,
                                  distance="Cosine") {
  
  if (is.na(vector_size)) {
    cli::cli_abort("You need to specify a numerical value for the vector size of the collection. It should match the size of the vectors generated by your embedding model. Aborting.")
  }
