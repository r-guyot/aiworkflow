...
myflow_pitch |>
  process_text_prompts("A new razor with 20 blades and a lot of springs for an ultimate and safe shaving experience aimed at adult men") |>
...
myflow_emoji |>
  process_text_prompts("The sun is shining today. Just the right temperature. I can finally relax and enjoy a good book.") |>
  pull_final_text_answer()
...
myflow_positive |> 
  ...
  process_text_prompts(company_report) |>
...
myflow_chained |> 
  ...
  process_text_prompts(text_prompts = company_report) |>
  pull_final_text_answer()

```

The only key difference is that you have no way to observe the intermediate results as they run when you do this.
...
Note that the quality of the Spanish translation in this case will depend heavily on how well the model you have selected can handle this kind of tasks. Not all models are equal when it comes to their multilingual capabilities.
